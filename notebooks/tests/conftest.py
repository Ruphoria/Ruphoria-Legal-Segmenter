...
def update_test_case(
    document_idx: int, expected_segment_count: t.Union[int, tuple[int, ...]]
) -> None:
    if not hasattr(expected_segment_count, "__len__"):
        expected_segment_count = (expected_segment_count,)

    assert (
        len(expected_segment_count) == len(REGISTERED_TEST_CASES_COLUMNS) - 1
    ), f"Got length: {len(expected_segment_count)}"
...
def print_results(
    ...
    if sentence and (
        not keyword_filters or not keyword_filters.isdisjoint(map(str.lower, sentence))
    ):
        print(c_color, segment_count, c_end, " ".join(sentence), end="\n\n")
