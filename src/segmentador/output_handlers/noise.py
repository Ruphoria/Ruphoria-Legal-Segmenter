...
def remove_noise_subsegments(
    ...
    *args: npt.NDArray[t.Any],
    ...
    maximum_noise_subsegment_length: t.Optional[int] = None,
) -> t.Tuple[npt.NDArray[np.int32], t.Tuple[npt.NDArray[t.Any], ...]]:
    """Remove noise subsegments."""
    ...
    max_noise_len = (
        maximum_noise_subsegment_length
        if maximum_noise_subsegment_length is not None
        else float("+inf")
    )
    ...
    for i_start, i_end in noise_subsegment_inds:
        if i_end - i_start <= max_noise_len:
